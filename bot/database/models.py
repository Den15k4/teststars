...
async def check_active_task(self, user_id: int) -> Tuple[bool, Optional[str], Optional[float]]:
    ...
    async with self.pool.acquire() as conn:
        ...
        # Если время последнего использования не установлено
        if not result['last_used']:
            await self.clear_pending_task(user_id)
            return False, None, None

        # Если задаче больше 30 минут
        age_seconds = result['age_seconds'] or 0
        if age_seconds > 1800:  # 30 минут
            await self.clear_pending_task(user_id)
            await self.return_credit(user_id)
            return False, None, None
